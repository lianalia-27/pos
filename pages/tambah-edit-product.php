...
if (isset($_POST['add'])) {
    ...
    if ($product_photo['error'] == 0) {
        ...
        $insertProduct = mysqli_query($koneksi, "INSERT INTO products (category_id, product_name, 
        product_price, qty, product_description, is_active, product_photo) VALUES ('$category_id', '$product_name', 
        '$product_price', '$qty', '$product_description', ' $is_active', '$fileName')");
        if ($insertProduct) {
...
category_id='$category_id',product_name='$product_name',$uploadFoto product_price='$product_price',qty='$qty',
product_description='$product_description',is_active='$is_active' 
WHERE id='$idEdit'");
        if ($update) {
            header("location:?page=product");
            exit;
